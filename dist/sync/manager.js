...
class SyncManager {
    ...
    async validateAndSync(schema) {
        try {
            ...
            logger_1.logger.success('Database validation and sync completed successfully');
        }
        catch (error) {
            logger_1.logger.error('Error during database validation and sync:', error);
            throw error;
        }
    }
    async syncModel(model) {
        try {
            ...
            if (missingFields.length > 0) {
                logger_1.logger.warn(`Missing fields in Notion database for model ${model.name}:`, missingFields);
                // 将来的な機能: データベースの自動更新
            ...
            if (typeErrors.length > 0) {
                logger_1.logger.error(`Type mismatches in model ${model.name}:`, typeErrors);
        ...
        for (const field of modelFields) {
            ...
            if (databaseProperty) {
                ...
                if (expectedType !== databaseProperty.type) {
                    typeErrors.push(`${notionName}: expected ${expectedType}, got ${databaseProperty.type}`);
                }
            }
        }
        return typeErrors;
    ...
    mapModelTypeToNotionType(modelType) {
        switch (modelType.toLowerCase()) {
            ...
            default:
                logger_1.logger.warn(`Unknown model type: ${modelType}, defaulting to rich_text`);
                return notionTypes_1.NotionPropertyTypes.RichText;
        }
    }
}
