...
export class QueryBuilder<T> {
  ...
  constructor(
    ...
    if (this.isDebugMode) {
      logger.debug(
        `クエリビルダーを初期化: ${modelName}, データベースID: ${databaseId}`
      );
      logger.debug(
        "プロパティマッピング:",
  ...
  where(
    ...
    if (this.isDebugMode) {
      logger.debug(
        `フィルター条件を追加: プロパティ=${mappedProperty}, 演算子=${operator}, 値=${JSON.stringify(
          value
        )}`
      );
    }
  ...
  orderBy(
    ...
    if (this.isDebugMode) {
      logger.debug(
        `ソート条件を追加: プロパティ=${mappedProperty}, 方向=${direction}`
      );
    }
    this.sorts.push({
      property: mappedProperty,
  ...
  private isRelationProperty(property: string): boolean {
    ...
    if (this.isDebugMode && isRelation) {
      logger.debug(
        `プロパティ "${property}" はリレーションとして検出されました`
      );
    }
    return isRelation;
  }
  ...
  private buildFilter(condition: FilterCondition | RelationFilter): any {
    if ("relationProperty" in condition) {
      if (this.isDebugMode) {
        logger.debug(
          `リレーションフィルターを構築: ${condition.relationProperty}, 値:`,
          condition.value
        );
      }
      return {
    ...
    if (this.isDebugMode) {
      logger.debug(
        `フィルターを構築: プロパティ=${property}, 演算子=${operator}, リレーション=${isRelation}, 値:`,
        value
      );
    }
  ...
  private buildSortCondition(sort: SortCondition): any {
    ...
    if (this.isDebugMode) {
      logger.debug(
        `ソート条件を構築: プロパティ=${mappedProperty}, 方向=${sort.direction}`
      );
    }
